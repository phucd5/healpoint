...
const BodySelector = () => {
  ...
  const handleSubmit = () => {
    if (body.length === 0) {
      ...
      const formattedBodyParts = body.map((part) => part.replace(/_/g, " "));
    ...
    for (const part in bothParts) {
      ...
      if (isPointInRectangle(x, y, x1, y1, x2, y2)) {
        ...
        if (body.includes(part)) {
          ...
          setBody((prevBody) =>
            prevBody.filter((item) => item !== part && item !== opposite)
          );
          ...
          setfrontHighlight((prevBody) => [...prevBody, part, opposite]);
          setbackHighlight((prevBody) => [...prevBody, part, opposite]);
        }
        break;
      }
    }
    ...
    for (const part in frontParts) {
      ...
      if (isPointInRectangle(x, y, x1, y1, x2, y2)) {
        if (body.includes(part)) {
          ...
          setBody((prevBody) => prevBody.filter((item) => item !== part));
          setfrontHighlight((prevBody) =>
